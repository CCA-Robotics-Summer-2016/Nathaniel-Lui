//Obstacle-Avoiding/Light-seeking Autonomatron 
...
const int RForward = 60;  //the speed of the servo, maximum speed is 180
...
void loop(){
  ...
  SensorLeft = 1023 - analogRead(LeftLightSensor);  //read photoresistors
  delay(1);
  SensorRight = 1023 - analogRead(RightLightSensor);
  delay(1);
  SensorDifference = abs(SensorLeft - SensorRight);
  
  if (distance > collisionThresh) //if path is clear seek brightest lightsource 
  {
